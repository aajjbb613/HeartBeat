#Get_SourceNotified.ps1
#This scripts connects to XXX where it runs a bash script that checks the local DB for the most recent fulfillment that has notified its source. if lack of chanage fires alerts.
...
$script= "/usr/local/bin/get_note_s.sh" 
$stateFile = "C:\Flags\get_note_s.last"

#runs script
$current   = (ssh XXX@XXX $script).Trim()

$hostname = hostname
$scriptname = "Get_SourceNotified"
...
if (Test-Path $stateFile) {
    ...
    if ($current -ne $previous -or $current -eq '0') {
        ...
        if($tag -eq "[HIGH]"){
            AS-CallBryce -Message "$tag : $scriptname : $hostname : fulfillment push. $current hasnt notified source" -NOQT
        }
        AS-SendEmail -Subject "$tag : $scriptname : $hostname : fulfillment push" -body "Order: $current hasnt notified source."
    }
} else {
